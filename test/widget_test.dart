...
void main() {
  testWidgets('Opener main flow with valid config',
    ...
    // add mocks
    final state =
        tester.state(find.byType(OpenerHomePage)) as OpenerHomePageState;
    state.opener = opener;
    state.storage = storage;
    // readCfg must be called again with fake storage
    state.readCfg();
    // XXX: why does "await tester.pumpAndSettle()" does not work here ?
    // XXX2: this used to take a Duration(milliseconds: 100)
    ...
    var labelStatus =
        find.byKey(Key("label_status")).evaluate().first.widget as Text;
